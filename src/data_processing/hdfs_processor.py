...
class HDFSProcessor(BaseProcessor):
    ...
    def create_sessions(self):
        ...
        data_df = pd.DataFrame(
            list(data_dict_content.items()), columns=["BlockId", "Content"]
        )
        ...
        data_df["Content"] = data_df["Content"].apply(
            lambda x: self.config["session_window_spliter"].join(x)
        )

        # Shuffle and split
        legacy_shuffle = bool(self.config.get("legacy_hdfs_shuffle", False))
        legacy_seed = self.config.get("legacy_seed")
        if legacy_shuffle:
            # Legacy: shuffle without a fixed seed (unless legacy_seed is provided).
            if legacy_seed is None:
                data_df = data_df.sample(frac=1).reset_index(drop=True)
            else:
                data_df = data_df.sample(
                    frac=1, random_state=int(legacy_seed)
    ...
    def tokenize_dataset(self):
        ...
        dataset = Dataset.from_pandas(
            df_normal[["Content"]].rename(columns={"Content": "text"})
        )
        ...
        keep_text = bool(self.config.get("legacy_keep_text_column", False))
        if keep_text:
