...
class Evaluator:
    ...
    def _load_test_data(self):
        ...
        balanced_df = pd.concat(
            [df[df["Label"] == 1].head(1000), df[df["Label"] == 0].head(1000)]
        )

        print(
            f"Loaded and balanced test data: {len(balanced_df[balanced_df['Label'] == 1])} anomaly, {len(balanced_df[balanced_df['Label'] == 0])} normal."
        )
    ...
    def _calculate_top_k_accuracy(self, model, tokenizer, texts=None):
        ...
        # Note: the legacy script does not call model.eval(); keep behavior aligned.
        ...
        with torch.no_grad():
            for i, text in enumerate(texts):
                if (i + 1) % 100 == 0:
                    print(
                        f"\r  Calculating accuracy (Antigo Method)... {i + 1}/{total_texts}",
                        end="",
                    )
                ...
                for k in self.config["top_k_values"]:
                    ...
                    # Legacy behavior (matches legado/federated_evaluation.py):
                    # checks membership against the whole top_k[0] matrix (seq_len x k),
    ...
    def evaluate(self):
        ...
        for round_num in range(1, self.config["num_rounds"] + 1):
            ...
            if threshold_mode == "fpr_target":
                ...
                if calib_texts:
                    calib_acc_df = self._calculate_top_k_accuracy(
                        model, tokenizer, calib_texts
                    )

            for k in self.config["top_k_values"]:
                scores: NDArray[np.float64] = np.asarray(
                    acc_df[f"top{k}"].to_numpy(dtype=float), dtype=np.float64
                )
                labels: NDArray[np.int64] = np.asarray(
                    acc_df["label"].to_numpy(dtype=int), dtype=np.int64
                )
                ...
                if threshold_mode == "fpr_target" and calib_acc_df is not None:
                    ...
                    calib_scores: NDArray[np.float64] = np.asarray(
                        calib_acc_df[f"top{k}"].to_numpy(dtype=float), dtype=np.float64
                    )
                    th = float(np.quantile(calib_scores, target))
                    preds = (scores < th).astype(int)
                    f1 = float(f1_score(labels, preds))
                else:
                    thresholds = np.linspace(
                        0, 1, int(self.config["f1_threshold_steps"])
                    )
                    best_f1 = -1.0
                    th = 0.0
